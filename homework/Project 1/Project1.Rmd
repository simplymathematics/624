...
hw3 <- hw(atm3, damped = FALSE)

hw2 <- hw(atm2.diff, damped=FALSE)


hw1 <- hw(atm1, damped=FALSE)
...
autoplot(hw3)
autoplot(hw4)
p1 <- predict(hw1, 30, prediction.interval=TRUE)
p2 <- predict(hw2, 30, prediction.interval=TRUE)
p3 <- predict(hw3, 30, prediction.interval=TRUE)
p4 <- predict(hw4, 30, prediction.interval=TRUE)
par(mfrow =c(2,2))
...
pred1  <- predict(arima1);
arima2 <- auto.arima(atm2, d=1);
pred2  <- predict(arima2);
arima3 <- auto.arima(atm3);
pred3  <- predict(arima3);
arima4 <- auto.arima(atm4);
pred4  <- predict(arima4);
...
atm1 <- as.data.frame(forecast(final1, 31))
atm2 <- as.data.frame(forecast(final2, 31))
atm3 <- as.data.frame(forecast(final3, 31))
atm4 <- as.data.frame(forecast(final4, 31))
...
writeMultipleData <- function (file, ...){
    ...
    for (i in 1:nobjects) {
        if (i == 1)
            write.xlsx(objects[[i]], file, sheetName = objnames[i], col.names = FALSE)
        else write.xlsx(objects[[i]], file, sheetName = objnames[i],
            append = TRUE, col.names = FALSE)
    }
  }

writeMultipleData("predictions/ATMs.xlsx", atm1, atm2, atm3, atm4)
```
